...
from api.services.TenantService import check_is_tenant
...
class UserController:
    ...
    def register_user():
        try:
            ...
            # Assign role
            role_result = assign_role(data["role"], result.inserted_id)
            if "error" in role_result:
                return jsonify({"error": role_result["error"]}), 400

            user_data["assigned_role"] = role_result.get("assigned_role")

            # Conditional tenant creation
            if data["role"] == "T":
                tenant_result = check_is_tenant(user_data, user_data)
                user_data["tenant"] = tenant_result  # Optional: attach to response
    ...
    def user_login():
        try:
            ...
            # Re-encode stored password (string in Mongo) to bytes for bcrypt check
